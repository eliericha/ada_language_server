...
Language specific features is possible [see here](#multi-language-handler)

### Graph

The main features of this extension is the graph.

There are several kind of graphs available:

#### Call Graph

The most basic kind of graph is the Call Graph which allows the user to visualize the relationship between the different function call of the project.
...
You can also generate Type Graphs which allows to visualize the hierarchies between the different type of your project.
...
You can also access graphs by using specific language. For example, when on an Ada file, you can right click anywhere in the editor and then on `Show File Dependency (graph)` to generate a graph representing the different files of your project and their relationship between them.

#### GPR Dependency Graph (GPR)

The last kind of graph visualizer tool available allows you to see the relationships between the different GPR files composing a project. Again, to use it you can click anywhere in a GPR file and then click on `Show GPR Dependency (graph)`.

The graph generated will be composed of different kind of nodes and edges :
For imported projects, the edges will be dotted.

For extended projects, the edges will be plain.

For aggregated projects, the node representing the aggregate project will be transformed in a sub-graph containing its aggregated project to better visualizer their relationship.

### Expansion

To prevent lag or slowness, the graph initially only display the required symbols and its direct relations but you can then interact with the buttons on the side of each node to either get the parents symbols or the children symbols for a specific node.
...
If the graph is disconnected, the program will try to relayout only the sub part that was modified.
In either case, when adding nodes the order of the node in the graph may be drastically reordered but the view will be focus on the node from which the other nodes were added to help the user keep track of its position in the graph.

You can also manually trigger the layouting by clicking the `Layout Graph` button on the bottom left side which will also change the direction of the graph on screen (toggling between top-bottom to left-right).
...
Once some characters have been entered, a list of node name will appears.
The user can then either click on a list item or use the `arrows` key or the `tab` key to go through the list.
...
-   `Get Super Hierarchy`: The label of this button change on the hierarchy of the graph and allows the user to request the parents of the node to be added to the graph, [see Expansion](#expansion)
-   `Get Sub Hierarchy`: The label of this button change on the hierarchy of the graph and allows the user to request the children of the node to be added to the graph, [see Expansion](#expansion)
-   `Go To References` : List the references of the symbol in the code base and then take the user to the chosen reference of the symbol in the code base, [see Go To Implementation](#go-to-implementation)
...
The user can also hold the `Control` key while deleting node to also delete all of their children recursively.
...
When modifying the code base while the graph is open, the location of some symbol might change, invalidating the node as LSP request depends on location.

To fix this issue the user can refresh the node by accessing it from the context menu under the label `Refresh Node`.

/!\ The tools was mainly developed as a way to visualize a code base, understand how different parts of the code interact with each other,...
It was not thought as a way to assist during active development so modifying the code base while the graph is open might invalidate a lot of nodes causing a part of the graph to not respond to request anymore.

### Jump To Code

When using the graph the user might want to see to which part of the code the node is related to.

To help with that, the application provides a few ways to access the code from the graph.

#### Go To Definition

First, by `double clicking` (or thought the context menu), the user will be taken to the definition of the symbol in the code base.
If the editor is already open, it will only be focused on the right position.
If the editor was not open, it will first be opened on a column next to the graph view.
...
-   First, the user can hover or click on an edge to open a popup containing a list representing all the references of the symbol at the destination of the edge in the context of the symbol at the source of the edge.
    If we take the Call Graph as an example (which is the most interesting), hovering an edge linking `Bar` to `Foo` would create a list all the function calls of `Foo` in the `Bar` function.
...
When going through the list, the user will see on a column on the side of the graph view, the code symbol represented by the list item revealed in real time, giving him context about what the symbol refers to.
...
To get more information on how to implement specific behavior, please check TODO A CONTRIBUTE.md FOR EXAMPLE
