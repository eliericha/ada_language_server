...
abstract class SequentialExecution extends vscode.CustomExecution {
     ...
     * prefix `ada: `
     */
    protected findTaskByName(tasks: vscode.Task[], taskName: string): vscode.Task {
...
class BuildAndRunExecution extends SequentialExecution {
    ...
    protected async getTasksToRun() {
        ...
        const buildMainTask = this.findTaskByName(
            adaTasks,
            this.buildAndRunDef.configuration.buildTask
        );
        const runMainTask = this.findTaskByName(
            adaTasks,
            this.buildAndRunDef.configuration.runTask
        );
...
class SequentialExecutionByName extends SequentialExecution {
    ...
    protected async getTasksToRun(): Promise<vscode.Task[]> {
        const adaTasks = await vscode.tasks.fetchTasks({ type: 'ada' });
        return this.taskNames.map((name) => this.findTaskByName(adaTasks, name));
    }
}
