...
with LSP.Ada_Handlers.Refactor.Extract_Expression;
...
package body LSP.Ada_Handlers is
   ...
   is
      ...
      is
         ...
         procedure Extract_Expression_Code_Action;
         --  Checks if the Extract Expression refactoring tool is available,
         --  and if so, appends a Code Action with its Command.
         ...
         ------------------------------------
         -- Extract_Expression_Code_Action --
         ------------------------------------

         procedure Extract_Expression_Code_Action
         is
            use LSP.Ada_Handlers.Refactor.Extract_Expression;
            use Langkit_Support.Slocs;
            ...
            Extract_Expression_Command : Command;

         begin
            if not Single_Location
              and then Is_Extract_Expression_Available (Node.Unit, SLOC)
            then
               Extract_Expression_Command.Append_Code_Action
                 (Context         => Context,
                  Commands_Vector => Result,
                  Where           =>
         ...
         end Extract_Expression_Code_Action;
   ...
   overriding function Get_Trace_Handle (Self : Message_Handler)
     return GNATCOLL.Traces.Trace_Handle is
       (LSP.GNATCOLL_Tracers.Handle (Self.Tracer.all));
